...
let getBlocks (diskMap: list<int>) : list<Option<int>> =
    let helper (index, value) =
        if index % 2 <> 0
        then [for _ in [0..value - 1] -> None]
        else [for _ in [0..value - 1] -> Some (int ((double index) / 2.0))]
...
let compressBlocks (blocks: list<option<int>>) =
    ...
    (blocks[..blocks.Length - numEmpty - 1]
    |> List.fold helper (0, []) |> snd) @ [for _ in [0..numEmpty - 1] -> None]

type File = {NumBlocks: int; Id: int}
type FreeSpace = {NumFreeBlocks: int; Files: list<File>}
...
let getDiskLocations (diskMap: list<int>) = 
    let helper (index, value) =
        if index % 2 <> 0
        then FreeSpace {NumFreeBlocks = value; Files = []}
        else File {NumBlocks = value; Id = (int ((double index) / 2.0))}
...
let compressFiles (diskLocations: list<DiskLocation>) =
    ...
    for i in [0..diskLocations.Length - 1] |> List.rev do
        ...
        | FreeSpace freeSpace -> ()
        | File file -> 
            let jOption: option<int> = Array.tryFindIndex (fun diskLoc -> match diskLoc with | FreeSpace freeSpace -> freeSpace.NumFreeBlocks >= file.NumBlocks | _ -> false) diskLocations[0..i-1]
            match jOption with
            | Some j -> 
                match diskLocations[j] with
                | File _ -> raise (System.InvalidOperationException "expected free space")
                | FreeSpace freeSpace ->
                    Array.set diskLocations j  (FreeSpace {NumFreeBlocks = freeSpace.NumFreeBlocks - file.NumBlocks; Files = freeSpace.Files @ [file]})
                    Array.set diskLocations i (FreeSpace {NumFreeBlocks = file.NumBlocks; Files = []})
            | None -> ()

    let helper diskLocation = 
        let processFile file = [for _ in [0..file.NumBlocks - 1] -> Some file.Id]
        match diskLocation with
        | File file -> processFile file
        | FreeSpace freeSpace -> (freeSpace.Files |> List.map processFile |> List.concat) @ [for _ in [0..freeSpace.NumFreeBlocks - 1] -> None]
