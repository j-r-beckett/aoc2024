...
let countXs (map: array<array<char>>) =
    let countXsInRow row =
        map[row]
        |> Array.fold (fun sumSoFar c -> sumSoFar + (if c = 'X' then 1 else 0)) 0
...
| Some guardPos ->
    ...
    let positions =
        ...
        |> List.filter (fun (row, col) -> map[row][col] = '.' && newMap[row][col] = 'X')
